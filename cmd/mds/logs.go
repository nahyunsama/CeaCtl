...
func addLogInputFlags(cmd *cobra.Command, input *logInputOptions) {
	cmd.Flags().StringVar(
		&input.fromStr,
		"from",
		"",
		"start date filter, YYYYMMDD (inclusive)",
	)
	cmd.Flags().StringVar(
		&input.toStr,
		"to",
		"",
		"end date filter, YYYYMMDD (inclusive)",
	)
	cmd.Flags().StringVar(
		&input.file,
		"file",
		"",
		"path to a local log file (skips device fetch)",
	)
}

func LogsCommand(opts *commandOptions) *cobra.Command {
	input := logInputOptions{}

	cmd := &cobra.Command{
		Use:   "logs",
		Short: "Fetch and compress MDS logs",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			from, err := parseDayStart(input.fromStr)
			if err != nil {
				return fmt.Errorf("invalid --from: %v", err)
			}
			to, err := parseDayEnd(input.toStr)
			if err != nil {
			...
			return writeCompressedLogReport(
				cmd.OutOrStdout(),
				reader,
				from,
				to,
	...
	addLogInputFlags(cmd, &input)
	cmd.AddCommand(LogsAnalyzeCommand(opts))

	return cmd
}

func writeCompressedLogReport(
	w io.Writer,
	reader io.Reader,
	from time.Time,
	to time.Time,
) error {
	result, err := logcompressor.Analyze(
		reader,
		from,
		to,
	)
	if err != nil {
		return fmt.Errorf("failed to compress log: %v", err)
	}

	if err := result.WriteReport(w, 10); err != nil {
		return fmt.Errorf("failed to write compressed log: %v", err)
	}
	return nil
}
